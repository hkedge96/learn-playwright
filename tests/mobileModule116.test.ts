...
test('Download modules', {tag: ['@regression', '@smoke']}, async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Menu' }).click();
  await page.locator('button').filter({ hasText: 'Menu' }).click(); // maybe i can use this in the if statement?
  ...
  await page.getByRole('complementary').getByRole('button', { name: 'Go' }).click();
